...
class PowerGridOptimizationApp(tk.Tk):
    ...
    def update_table(self, event=None):
        ...
        if self.object_var.get() == "Узлы":
        ...
        elif self.object_var.get() == "Линии":
            ...
            for p in self.mdl.bp:
                ...
                self.tree.insert("", "end", values=(p.name, from_node, to_node,
                                                    ...
                                                    p.desc))
        elif self.object_var.get() == "Взаимоиндукции":
            self.tree["columns"] = ["Название", "От линии", "К линии", "M12", "M21"]
            self.tree.heading("Название", text="Название")
            self.tree.heading("От линии", text="От линии")
            self.tree.heading("К линии", text="К линии")
            self.tree.heading("M12", text="M12")
            self.tree.heading("M21", text="M21")

            # Ограничиваем ширину столбцов  
            self.tree.column("Название", width=100, minwidth=50)
            self.tree.column("От линии", width=100, minwidth=50)
            self.tree.column("К линии", width=100, minwidth=50)
            self.tree.column("M12", width=100, minwidth=50)
    ...
    def add_object(self):
        if self.object_var.get() == "Узлы":
            ...
            E = self.ask_complex_tuple("Введите ЭДС E (E1,E2,E0) в Вольтах (необязательно)", (0, 0, 0))
            B = self.ask_complex_tuple("Введите емкостную проводимость B (B1,B2,B0) в См (необязательно)", (0, 0, 0))
            ...
            name = simpledialog.askstring("Добавить взаимную индукцию", "Введите название взаимной индукции:")
            if not name:
                return
            
            line1_name = simpledialog.askstring("Добавить взаимную индукцию", "Введите название первой линии:")
            line2_name = simpledialog.askstring("Добавить взаимную индукцию", "Введите название второй линии:")
            
            line1 = next((p for p in self.mdl.bp if p.name == line1_name), None)
            line2 = next((p for p in self.mdl.bp if p.name == line2_name), None)
            
            if not (line1 and line2):
                tk.messagebox.showerror("Ошибка", "Одна или обе линии не найдены")
                return
            
            M12 = self.ask_imag("Введите взаимную индукцию M12:")
            M21 = self.ask_imag("Введите взаимную индукцию M21:")
            
            mrtkz.M(self.mdl, name, line1, line2, M12, M21)

        self.update_table()
        self.update_visualization()
        self.update_subregimes_visualization()

    def ask_imag(self, prompt, default=None):
        result = []
        while True:
            value = simpledialog.askstring(prompt, f"Введите значение (без j):")
            ...
            try:
                ...
                tk.messagebox.showerror("Ошибка", "Неверный формат. Введите одно число.")
        return imag
    
    def ask_complex_tuple(self, prompt, default=None, values=3):
        result = []
    ...
    def delete_object(self):
        ...
        if self.object_var.get() == "Узлы":
            ...
            self.mdl.bm.remove(next(m for m in self.mdl.bm if m.name == values[0]))  # Удаление взаимной индукции
    ...
    def start_optimization(self): 
        # обновляем список значений графика
        self.loss_values = []
        ...
        # добавление недостающих ступеней
        for p in self.mdl.bp:
            if p.q1 != 0:
               q1_stages =  [d.stage for d in p.q1_def]
               for i in range(1,5):
                   if not i in q1_stages:
                       mrtkz.protection(p=p, q=p.q1, stage=i, I0=999999, t=9999999, type='ТЗНП', P_rnm=0, rnm_base_angle=0, stage_on = True, I0_range=[300,3000], t_range=[0,20], desc='')
            else: pass
            if p.q2 != 0:
               q2_stages =  [d.stage for d in p.q2_def]
               for i in range(1,5):
                   if not i in q2_stages:
                       mrtkz.protection(p=p, q=p.q2, stage=i, I0=999999, t=9999999, type='ТЗНП', P_rnm=0, rnm_base_angle=0, stage_on = True, I0_range=[300,3000], t_range=[0,20], desc='')
            else: pass                                  
